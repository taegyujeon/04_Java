 ...
 */
	private void todoAdd() throws IOException {
		...
		while(true) {
			...
			if(content.equals("!eq")) break; // !wq 입력 반복 종료
  ...
  //---------------------------------------------------------------------------------------------
	...
	/**
	 * 할 일 수정
	 * - 인덱스 번호를 입력 받아 정상 범위내에 index 값인지 확인
	...
	private void todoUpdate() throws NumberFormatException, IOException {
		...
		System.out.println("수정 할 제목 입력: ");
		String title = br.readLine();
		
		System.out.println("수정 할 세부 내용 작성(입력 종료시 !wq 작성 후 엔터)");
		System.out.println("--------------------------------------------");
		...
		boolean result = service.todoUpdate(index, title, toString());
